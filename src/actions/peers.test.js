...
describe('actions: peers', () => {
  ...
  describe('activePeerSet', () => {
    ...
    it('should set to testnet if not defined in config but port is 7000', () => {
      const network9305 = { address: 'http://127.0.0.1:9305', nethash };
      const network9405 = { address: 'http://127.0.0.1:9405', nethash };

      activePeerSet({ passphrase, network: network9305 })(dispatch);
      expect(dispatch).to.have.been.calledWith(match.hasNested('data.activePeer.options.testnet', true));

      activePeerSet({ passphrase, network: network9405 })(dispatch);
